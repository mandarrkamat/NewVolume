...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Floating button permission check
        if (!Settings.canDrawOverlays(this)) {
            new AlertDialog.Builder(this)
                .setTitle("Permission आवश्यक आहे")
                .setMessage("Floating button साठी 'Display over other apps' permission द्या.")
                .setPositiveButton("Settings उघडा", (d, w) -> {
                    Intent intent = new Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION,
                        Uri.parse("package:" + getPackageName()));
                    startActivityForResult(intent, 1234);
                })
                .setNegativeButton("Skip", (d, w) -> showVolumeControlDialog())
                .show();
        } else {
    ...
    private void showVolumeControlDialog() {
        ...
        if (currentDialog.getWindow() != null) {
            currentDialog.getWindow().setDimAmount(0.6f);
            currentDialog.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
            WindowManager.LayoutParams lp = new WindowManager.LayoutParams();
            lp.copyFrom(currentDialog.getWindow().getAttributes());
            lp.width = (int)(getResources().getDisplayMetrics().widthPixels * 0.92f);
            currentDialog.getWindow().setAttributes(lp);
        }
        currentDialog.setOnDismissListener(d -> finish());
        currentDialog.show();
    }
    ...
    private void rebootDevice() {
        new AlertDialog.Builder(this)
            ...
            .setPositiveButton("Reboot", (d, w) -> {
                try { ((PowerManager)getSystemService(POWER_SERVICE)).reboot(null); }
                catch (Exception e) {
